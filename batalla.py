...
def _pantalla_simple(lineas, sonido):
    sonido.play()
    while True:
        surf = configuracion.get_surface_juego()
        surf.fill(configuracion.NEGRO)
        for i,(msg,color) in enumerate(lineas):
            r = _f22.render(msg, True, color)
            surf.blit(r, r.get_rect(center=(W//2, int(H*(0.20 + i*0.14)))))
        configuracion.presentar(pygame.display.get_surface(), surf)
...
def pantalla_victoria(nivel, errores=0, total=0):
    lineas = [
        ("EXAMEN APROBADO",              configuracion.VERDE),
        (f"Avanzas al nivel {nivel+1}",  configuracion.BLANCO),
    ]
    if total > 0:
        color_e = configuracion.ROJO if errores > 0 else configuracion.VERDE
        lineas.append((f"Errores en este nivel: {errores} / {total}", color_e))
    lineas.append(("ENTER para continuar", configuracion.BLANCO))
    _pantalla_simple(lineas, victoria_sound)

def pantalla_derrota(nivel, errores=0, total=0):
    msgs = {1: "El profesor cree que no estudiaste.",
            2: "Fuiste enviado al sustitutorio.",
            3: "El examen final destruyo tu promedio."}
    lineas = [
        ("DERROTA",       configuracion.ROJO),
    ...
    _pantalla_simple(lineas, derrota_sound)
    pygame.quit(); sys.exit()

def pantalla_final(errores_por_nivel=None):
    lineas = [
        ("FELICITACIONES",               configuracion.VERDE),
        ("APROBASTE HISTORIA DEL PERU",  configuracion.BLANCO),
    ]
    if errores_por_nivel:
        ...
        for niv, (e, t) in sorted(errores_por_nivel.items()):
            ...
            lineas.append((f"Nivel {niv}: {e} errores de {t} preguntas", color_e))
        color_tot = configuracion.ROJO if total_e > 0 else configuracion.VERDE
        lineas.append((f"Total: {total_e} errores / {total_p} preguntas", color_tot))
    lineas.append(("ENTER para salir", configuracion.BLANCO))
    _pantalla_simple(lineas, victoria_sound)
    pygame.quit(); sys.exit()
...
def batalla(nivel, preguntas_custom=None):
    ...
    while True:
        ...
        if segs <= 0:
            ...
            vida_est -= 10
            if vida_est <= 0:
                return False, errores, respondidas
            indice = (indice+1) % len(preguntas)
            if indice == 0:
                if preguntas_custom:
                    # Se acabaron las preguntas: gana quien tenga más vida
                    return vida_prof < vida_est, errores, respondidas
                preguntas = obtener_preguntas(nivel)
            pregunta = preguntas[indice]
            opciones = pregunta["opciones"].copy(); random.shuffle(opciones)
            t0 = pygame.time.get_ticks()
            ...
            if ev.type == pygame.MOUSEBUTTONDOWN:
                ...
                for boton, opcion in bots:
                    if boton.collidepoint(pos_l):
                        ...
                        if vida_prof <= 0:
                            return True, errores, respondidas
                        if vida_est  <= 0:
                            return False, errores, respondidas
